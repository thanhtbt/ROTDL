...
tucker_rank = [3 3 3];
epsilon     = 0e-3*ones(time_frame,1);
epsilon(300)= 1;
outlier_den = 0.1;
outlier_fac = 0;

fac_noise_vec = [1e-0 1e-1 1e-2];


PER_SEP = zeros(length(fac_noise_vec),time_frame);

for jj = 1 : length(fac_noise_vec)
     
    fac_noise = fac_noise_vec(jj);
...
lgd = legend([d12 d22 d32 ],'$\sigma_n = 1$','$\sigma_n = 0.1$','$\sigma_n = 0.01$');
lgd.FontSize = 28;
