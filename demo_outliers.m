...
tensor_dim  = [10 15 20];                 % dimension of temporal slices
tucker_rank = [3 3 3];
fac_noise   = 1e-3;
outlier_den = 0.1;
% outlier_fac = 10;
...
outlier_fac_vec = [0.1 1 10];

PER_SEP = zeros(length(outlier_fac_vec),time_frame);

for jj = 1 : length(outlier_fac_vec)
    outlier_fac = outlier_fac_vec(jj);
...
lgd = legend([d12 d22 d32],'$\texttt{fac} = 0.1$','$\texttt{fac} = 1$','$\texttt{fac} = 10$');
lgd.FontSize = 28;
